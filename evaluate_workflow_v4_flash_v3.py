...
This is a new compatibility-safe path.  Historical v1/v2 runners and reports
remain frozen.  The model receives the complete public device and event
interfaces, exactly four action kinds, and no private Episode schedule.
"""
...
from evaluate_workflow_v4_flash import REQUEST_BODY_FIELDS
...
DEFAULT_OUTPUT = ROOT / "runs/workflow_v4_flash_complete_prompt_event_api_20_v3/report.json"
...
PROTOCOL_RULES = [
    ...
    "The observable_event_catalog is global and exhaustive. Use only its filterable scalar fields in wait.until_event; it does not reveal which events will occur in this Episode.",
...
def build_messages(view: dict[str, Any], previous_action: dict[str, Any] | None = None) -> list[dict[str, str]]:
    payload = {
...
class WorkflowLLMPolicy:
    ...
    def decide(self, view: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            response = self.client.complete(build_messages(view, self.previous_action))
...
def build_report(results: list[dict[str, Any]], indices: list[int], *, execution_finished: bool, baseline: dict[str, Any]) -> dict[str, Any]:
    ...
    if result_ids != expected_ids:
        raise RuntimeError("v3 result Episode IDs do not exactly match the frozen paired sample")
    return {
        "schema_version": "temporal-home-v4-flash-complete-event-api-v3",
        ...
        "experiment_config": {
            ...
            "changed_vs_v2": ["temporal_home_backend", "temporal_home_evaluator", "four_action_api", "complete_observable_event_catalog", "previous_action_context", "device_command_count_vocabulary"],
...
def main(args: list[str] | None = None) -> dict[str, Any]:
    ...
    if ns.base_url != DEFAULT_BASE or ns.model != DEFAULT_MODEL:
        raise ValueError("paired v3 rerun freezes the default base URL and model")
    ...
    if indices != FIXED_INDICES:
        raise ValueError("the paired v3 rerun requires exactly the fixed indices 0,10,...,190")
